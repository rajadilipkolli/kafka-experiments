...
public class KafkaConfiguration implements KafkaListenerConfigurer {
    ...
    public NewTopic deadLetterTopic(AppKafkaProperties properties) {
        ...
        return TopicBuilder.name(ORDERS + properties.deadletter().suffix())
                ...
                .partitions(1)
                // Use longer retention for Dead Letter Topic, allowing for more time to
                // troubleshoot
                .config(
                        TopicConfig.RETENTION_MS_CONFIG,
                        "" + properties.deadletter().retention().toMillis())
                .build();
    }

    @Bean
    public DefaultErrorHandler defaultErrorHandler(
            KafkaOperations<Object, Object> operations, AppKafkaProperties properties) {
        ...
        var recoverer =
                new DeadLetterPublishingRecoverer(
                        ...
                        (cr, e) ->
                                new TopicPartition(
                                        cr.topic() + properties.deadletter().suffix(), 0));

        // Spread out attempts over time, taking a little longer between each attempt
        // Set a max for retries below max.poll.interval.ms; default: 5m, as otherwise we trigger a
        // consumer rebalance
